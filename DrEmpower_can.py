...
class DrEmpower_can(object):
        ...
        try:
            if len(id_list) == len(angle_list):
                if mode == 0:
                    ...
                    if len(self.cur_angle_list) != len(angle_list) or self.set_angles_mode_1_flag == 0:
                        self.cur_angle_list = []
    ...
    def enable_angle_speed_torque_state(self, id_num=0):
        ...
        try:
            self.write_property(id_num, 'dr.can.config.enable_state_feedback', 1)
    ...
    def disable_angle_speed_torque_state(self, id_num=0):
        ...
        try:
            for i in range(5):
                self.write_property(id_num, 'dr.can.config.enable_state_feedback', 0)
            time.sleep(0.5)
    ...
    def read_property(self, id_num=0, property=''):
        ...
        try:
            ...
            # print(time.time(), 'sended_command')
            cdata = self.receive_data()
            # print(time.time(), 'receive_data')
            if READ_FLAG == 1:
