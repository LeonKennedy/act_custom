...
property_addresss = {
    ...
    'dr.config.state_feedback_rate_ms': 31000 + plus_plus(1),
    # CAN总线实时状态反馈时间间隔，默认为 2，单位 ms，当总线中不同 ID 号关节数量为 n 时，需将该值设置为 2n
    ...
    'dr.config.enable_encoder_circular_limit': 31200 + plus_plus(1),
    # 是否开启多圈计数角度限制，超出该限制关节将无法记住重启前的角度，若关闭该限制则务必将编码器供电线拔掉，即默认系统不需要关机后零点位置
    # dr.controller. #
    ...
    'dr.controller.config.speed_limit_tolerance': 32100 + plus_plus(1),
    # 超出最大转速的忍耐度，比如设定为 1.2 时表示当转速超过设定的最大转速值的 1.2 倍才会触发 超速错误
